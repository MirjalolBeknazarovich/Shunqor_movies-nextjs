...
const Account = ({subscription}: AccountProps) => {
  ...
  return (
    ...
    <main className="mx-auto max-w-6xl px-5 pt-24 pb-12 transition-all md:px-12">
      <div className="flex flex-col gap-x-4 md:flex-row md:items-center">
        <h1 className="text-3xl md:text-4xl"> Account</h1>
        <div className=" -ml-1 flex items-center gap-x-1.5">
          <MdOutlineSubscriptions className="w-5 h-5 text-red-500" />
          <p className="text-md font font-semibold text-[#555] ">Member since { moment( subscription.current_period_start * 1000).format("DD MMM yyyy") }</p>
        </div>
      </div>
      ...
      <div className='mt-6 grid grid-cols-1 gap-x-4 border px-4 py-4 md:grid-cols-4 md:bordder-x-0 md:border-t md:border-b-0 md:pb-0'>
        ...
        <p className="cursor-pointer col-span-3 text-blue-500 hover:underline md:text-right">Change Plan</p>
      </div>
      <div className='mt-6 grid grid-cols-1 gap-x-4 border px-4 py-4 md:grid-cols-4 md:bordder-x-0 md:border-t md:border-b-0 md:pb-0'>
        <h4 className="text-lg text-[gray]">Settings</h4>
        <p onClick={logout} className="cursor-pointer col-span-3 text-blue-500 hover:underline">Sign out of all devices</p>
      </div>
    </main>
  </>
  )
...
export const getServerSideProps: GetServerSideProps<AccountProps> = async ({ req }) => {
  ...
  const subscription = await fetch(`${API_REQUEST.subscription}/${user_id}`).then(response => response.json())
